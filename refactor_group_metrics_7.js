...
const targetStr = `<div key={idx} className="bg-white dark:bg-surface p-2 rounded-xl border border-border shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow relative overflow-hidden">
...
const replacementStr = `<div key={idx} className="bg-white dark:bg-surface p-2 rounded-xl border border-border shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow relative overflow-hidden">
                                                                    <div className="text-xs text-ink-muted font-medium mb-1 line-clamp-2 leading-tight" title={base.semanticMeaning}>
                                                                        {base.originalName || base.semanticMeaning}
                                                                    </div>
                                                                    
                                                                    <div className="mt-auto flex flex-col items-start gap-0.5 w-full">
                                                                        {progress ? (
                                                                            <div className="inline-flex items-center justify-center bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 font-bold px-1.5 py-0.5 rounded-md" style={{ fontSize: '10px' }}>
                                                                                {progress.numericValue !== null ? progress.numericValue : progress.rawValue}
                                                                                {progress.unit ? progress.unit : "%"}
                                                                            </div>
                                                                        ) : (
                                                                        ...
                                                                        <div className="flex items-baseline gap-0.5 whitespace-nowrap">
                                                                            {(() => {
                                                                                ...
                                                                                const isText = typeof val === 'string' && isNaN(Number(val));
                                                                                return (
                                                                                    <span className={\`font-bold text-ink \${isText ? '' : 'text-base sm:text-lg'}\`} style={{ fontSize: isText ? '11px' : undefined }} title={isText ? String(val) : undefined}>
                                                                                        {val}
                                                                                    </span>
                                                                                );
                                                                            })()}
                                                                            {base.unit && (
                                                                                <span className="font-medium text-ink-muted" style={{ fontSize: '10px' }}>{base.unit}</span>
                                                                            )}
                                                                        </div>
                                                                    </div>
                                                                </div>`;
...
if (c.includes(targetStr)) {
    ...
    console.log('Successfully fixed font sizes!');
} else {
