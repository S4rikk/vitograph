...
if (c.includes(importLucideTarget) && !c.includes('Clock, MoreHorizontal')) {
    c = c.replace(importLucideTarget, '    Clock,\n    MoreHorizontal,\n    TrendingUp,\n    Zap,\n} from "lucide-react";');
}

// 2. Replace the whole rendering logic for finalGroups with the new Garmin-style widget layout
const renderTargetStart = 'const rawMetrics = Object.values(latestSemanticMetrics) as any[];';
const renderTargetEnd = '})()}';

const startIdx = c.indexOf(renderTargetStart);
const endIdx = c.indexOf(renderTargetEnd, startIdx) + renderTargetEnd.length;

if (startIdx !== -1 && endIdx !== -1) {
    const newRenderLogic = `const rawMetrics = Object.values(latestSemanticMetrics) as any[];
                                                                    ...
                                                                    <div className="flex-1 bg-gradient-to-br from-surface to-surface/90 p-4 rounded-3xl border border-white/5 relative overflow-hidden shadow-lg shadow-black/20 flex flex-col justify-between">
                                                                        ...
                                                                        {nextWorkout && (
                                                                            <div className="flex items-center justify-between bg-black/20 p-3 rounded-2xl border border-white/5 mb-4">
                                                                                ...
                                                                                <div className="w-8 h-8 rounded-full border-2 border-blue-400 border-t-transparent animate-spin-slow flex items-center justify-center">
    ...
    c = c.substring(0, startIdx) + newRenderLogic + c.substring(endIdx);
    fs.writeFileSync('C:/project/VITOGRAPH/apps/web/src/components/profile/UserProfileSheet.tsx', c);
    console.log('Successfully applied full Garmin widget architecture!');
} else {
    console.log('Target rendering block not found!');
}
