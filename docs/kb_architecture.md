...
> **Дата актуальности:** 24 апреля 2026
...
- **Иерархический chunking:** document → section → chunk (parent-child retrieval) с улучшенным батчингом
- **Гибридный поиск:** tsvector (лексический) + pgvector (семантический) + RRF fusion
- **Батч-ингестия:** Edge Function с обработкой по 50 чанков (batch queue processing) через `pgmq` и `kb_ingest_enqueue_batch` RPC
...
```mermaid
graph TB
    subgraph "Ингестия (Async Batching)"
        INS["INSERT kb_documents"]
        TRG["DB Trigger"]
        Q["pgmq.send('kb_ingest')"]
        CRON["pg_cron (30s) / Self-requeue"]
        EF["Edge Function: kb-ingest (Batch 50)"]
