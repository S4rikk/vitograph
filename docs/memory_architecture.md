...
## 12. Memory Cleanup & Erasure (Clear Long-Term Memory)

В приложении реализована функция "Очистить память ИИ" (Clear AI Memory), которая позволяет пользователю в один клик сбросить все накопленные данные о себе. Это необходимо для обеспечения приватности и возможности начать взаимодействие "с чистого листа".

### Endpoint: `DELETE /api/v1/ai/memory/long-term`
Контроллер: `handleClearLongTermMemory` (`ai.controller.ts`).

### Механика удаления
Операция выполняется от лица авторизованного пользователя с соблюдением политик RLS (Row Level Security). Удаление происходит каскадно (или параллельно) из следующих таблиц:
1. `user_memory_vectors` — удаляются все семантические векторы (факты, предпочтения, страхи, опыт).
2. `user_emotional_profile` — удаляется весь эмоциональный профиль пользователя (L3).
3. `memory_consolidation_log` — очищаются логи работы background-консолидатора.

> **Примечание:** Операционная память (L1) LangGraph (таблица `checkpoints`) очищается отдельно в рамках еженедельного cron-job `Checkpoint Pruning`, либо удаляется при удалении всего аккаунта пользователя. Очистка долгосрочной памяти сбрасывает именно "знания" ИИ, но не историю диалогов.

---

## 13. Data Flow (Complete)
