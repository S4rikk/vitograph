...
const replacement = `                                                <div className="flex w-full gap-2">
                                                    <button
...
// Instead of exact string, let's use replace with regex.
// We are looking for the 'Ввести вручную' button and the closing </div>
const match = c.match(/<button[\s\S]*?Ввести вручную[\s\S]*?<\/button>\s*<\/div>/);
if (match) {
