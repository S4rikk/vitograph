...
const stateLogic = `    const [isEditingMetrics, setIsEditingMetrics] = useState(false);
    ...
    const handleDeleteMetric = async (metricKey: string) => {
        ...
        try {
            ...
            const { data: rows } = await supabase.from('wearable_manual_metrics').select('*').eq('user_id', userId);
            if (!rows) return;
...
if (!c.includes("isEditingMetrics")) {
    c = c.replace(/    const handleSaveProfile = async \(e\?: React\.FormEvent\) => \{/, stateLogic + "\n    const handleSaveProfile = async (e?: React.FormEvent) => {");
}
...
// 4. Inject exact Garmin render block
const startStr = '{Object.keys(latestSemanticMetrics).length === 0 ? (';
const endStr = '                                    </TabsContent>';
const startIndex = c.indexOf(startStr);
const endIndex = c.indexOf(endStr, startIndex);

if (startIndex > -1 && endIndex > -1) {
    const renderBlockStr = fs.readFileSync('C:/project/VITOGRAPH/perfect_render_block.txt', 'utf8');
    const finalBlock = renderBlockStr + `
                                            )}
                                        </div>
`;
    c = c.slice(0, startIndex) + finalBlock + c.slice(endIndex);
    fs.writeFileSync(path, c);
    console.log("Successfully replaced complete Garmin render logic with v2!");
} else {
