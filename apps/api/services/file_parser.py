...
class ReferenceRange(BaseModel):
    ...
    text: Optional[str] = Field(
        None, description="Original text of the norm, e.g., '< 15' or 'Negative'"
    )
...
class BiomarkerResult(BaseModel):
    ...
    value_string: Optional[str] = Field(
        None, description="To be used if result is textual (e.g., Negative, Trace)"
    )
    unit: Optional[str] = Field(None, description="Measurement unit exactly as stated")
    reference_range: Optional[ReferenceRange] = Field(
        None, description="Reference range data"
    )
    flag: Optional[str] = Field(
        None, description="Normal, Low, High, Abnormal. Calculated strictly from text!"
    )
    ai_clinical_note: Optional[str] = Field(
        None,
        description="Brief explanation (1 sentence) of what this marker represents.",
    )
...
class LabReportExtraction(BaseModel):
    ...
    context: Optional[str] = Field(
        None, description="E.g., morning, fasting, cycle phase, etc."
    )
    total_found_count: int = Field(
        0, description="The exact number of biomarker rows detected in the document."
    )
...
async def extract_biomarkers(
    file_bytes: bytes, filename: str, locale: str = "ru"
) -> LabReportExtraction:
    ...
    system_prompt = (
        ...
        "2. DO NOT TRUNCATE: Extract EVERY single biomarker listed in the text. Do not skip any rows. "  # noqa: E501
        "Even 'Normal' findings are critical for the clinical history. A full panel might contain 50+ items.\n"  # noqa: E501
        "3. FAIL-SAFE: If the text provided does not resemble a medical lab report (e.g., a random story, "  # noqa: E501
        "a recipe, or a blank page), return an empty array for `biomarkers: []` and 0 for `total_found_count`."  # noqa: E501
    )
...
async def extract_biomarkers_from_image(
    image_bytes: bytes, content_type: str, locale: str = "ru"
) -> LabReportExtraction:
            ...
            messages=[
                {"role": "system", "content": system_prompt},
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": "Extract all biomarker data from this lab report photo:",  # noqa: E501
                        },
                        {
                            "type": "image_url",
...
async def extract_biomarkers_from_image_batch(
    images: List[Tuple[bytes, str]], locale: str = "ru"
) -> LabReportExtraction:
    ...
    tasks = [
        extract_biomarkers_from_image(image_bytes, content_type, locale=locale)
